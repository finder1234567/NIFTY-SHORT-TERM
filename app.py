...
def process_scoring(ind, long_term):
    # ========== SHORT-TERM MOMENTUM SCORE (5-20 days) ==========
    st_score = 50.0
    if ind["rsi"] < 30:
        st_score += 15
    elif ind["rsi"] > 70:
    ...
    st_score = max(0.0, min(100.0, st_score))
    st_rec = "HOLD"
    if st_score >= 65:
        st_rec = "BUY"
    elif st_score <= 35:
    ...
    # ========== COMBINED VERDICT LOGIC WITH SHORT-TERM ==========
    if st_rec == "BUY" and lt_rec == "BUY" and ult_rec == "BUY":
        combined_verdict = "🚀 STRONG BUY (All Signals Aligned)"
        absolute_rec = "BUY (High Conviction)"
    elif st_rec == "BUY" and lt_rec == "BUY":
        combined_verdict = "📈 SOLID BUY (ST+LT Bullish)"
        absolute_rec = "BUY (Add Position)"
    elif st_rec == "BUY" and ult_rec == "BUY":
        combined_verdict = "💎 MOMENTUM BUY (ST Momentum, ULT Strong)"
        absolute_rec = "BUY (Trade Entry)"
    elif st_rec == "SELL" and lt_rec == "BUY" and ult_rec == "BUY":
        combined_verdict = "🔄 BUY on Dips (LT+ULT Bullish)"
        absolute_rec = "BUY (On Pullback)"
    elif ult_rec == "BUY" and (st_rec == "SELL" or lt_rec == "SELL"):
        combined_verdict = "💎 ACCUMULATE (3Y Bullish)"
        absolute_rec = "BUY (Long-term Hold)"
    elif st_rec == "BUY" and ult_rec == "SELL":
        combined_verdict = "⚠️ CAUTIOUS BUY (ST Hot, ULT Weak)"
        absolute_rec = "HOLD (Take Profits)"
    elif ult_rec == "SELL":
        combined_verdict = "🔴 AVOID (ULT Bearish)"
        absolute_rec = "SELL (Downtrend)"
    elif st_rec == "SELL" and lt_rec == "SELL":
        combined_verdict = "📉 SELL (Short-term Weakness)"
        absolute_rec = "SELL (Momentum Loss)"
    else:
        combined_verdict = f"→ {st_rec}"
        absolute_rec = st_rec
    
    return {
        "short_term_score": round(st_score, 1), "short_term_rec": st_rec,
...
def scan_single_ticker(ticker, name):
    try:
        ...
        return {
            ...
            "Support": f"₹{long_term['support']:,.2f}",
...
def highlight_recommendations(val):
    if isinstance(val, str):
        ...
        elif "SOLID BUY" in val or "MOMENTUM BUY" in val:
            return 'background-color: #0d7338; color: #ffffff; font-weight: bold;'
        elif "BUY on Dips" in val or "ACCUMULATE" in val:
            return 'background-color: #cce5ff; color: #004085; font-weight: bold;'
        elif "CAUTIOUS BUY" in val:
            return 'background-color: #fff3cd; color: #856404; font-weight: bold;'
        elif ("BUY" in val.upper() and "REC" not in val):
...
def main():
    ...
    st.caption("📊 SHORT-TERM (5-20d) + LONG-TERM (1Y) + ULTRA LONG-TERM (3Y) Analysis")
    
    tickers_all = get_nifty50_tickers()
    with st.sidebar:
        st.header("⚙️ Scanning Framework")
        subset_n = st.slider("Universe Size", 5, len(tickers_all), min(25, len(tickers_all)))
        max_workers = st.slider("Parallel Threads", 2, 16, 8)
        run_btn = st.button("🔍 SCAN STOCKS", type="primary", use_container_width=True)
    ...
    if run_btn:
        ...
        def _cb(d, t):
            progress.progress(d / t, text=f"Scanned {d}/{t} stocks...")
    ...
    if data:
        ...
        st.divider()
        st.subheader("📌 Signals Explanation")
        ...
        with col2:
            st.success("💎 **MOMENTUM BUY** - ST hot + ULT strong")
            st.success("💎 **ACCUMULATE** - ULT bullish, build position")
            st.warning("⚠️ **CAUTIOUS BUY** - ST hot but ULT weak")
        with col3:
        ...
        solid_buy = df[df["Combined Strategy"].str.contains("SOLID BUY|MOMENTUM BUY", na=False)]
        accumulate = df
